...
class LidarDriver:
    """
    lidar.data playback driver:
    - decode_lidar_stream ile dosyadan frame okur
    - LidarProjectionSystem ile world + filtre uygular
    - sonucu callback ile UI pipeline'a verir
    """
    ...
    async def run_async(self):
        print("[LidarDriver] lidar.data playback başladı")
        ...
        try:
            while self._running:
                ...
                for ts, frame in decode_lidar_stream(self.file_path):
                    ...
                    # CALLBACK: servise aktar
                    self.on_points(world_pts)

                    # Debug (seyrek)
                    if self.debug and (frame_index % self.debug_every_n == 0):
                        print(f"[LidarDriver] FRAME {frame_index} | ts={ts:.3f}")
                        print(f"[LidarDriver] raw points  : {cart.shape}")
                        print(f"[LidarDriver] world points: {world_pts.shape}")
                        if world_pts.size > 0:
                            p0 = world_pts[0]
                            print(
                                f"[LidarDriver] first point → "
                                f"x={p0[0]:.2f}, y={p0[1]:.2f}, z={p0[2]:.2f}"
                            )
                        else:
                            print("[LidarDriver] world points EMPTY")
            ...
            print("[LidarDriver][FATAL] exception:", repr(e))
        finally:
