...
class ViewerPanel:
    def __init__(self, master, projection_system, viewer_canvas, config_manager, lidar_service=None, lidar_projection=None):
        self.master = master
        self.ps = projection_system          # camera projection system
    ...
    def _load_from_config(self):
        ...
        # Lidars (ConfigManager.lidars -> list[dict])
    ...
    def _apply_lidar_settings(self):
        ...
        # canlı modele uygula (varsa)
        # ✅ canlı modele uygula: projeksiyon sistemindeki lidar model
        if self.lidar_ps and getattr(self.lidar_ps, "lidar", None):
            self.lidar_ps.lidar.set_transform(x, y, z, yaw, pitch, roll)


        if hasattr(self.config, "save_lidars"):
            self.config.save_lidars()
        elif hasattr(self.config, "save_lidar"):
    ...
    def _build_ui(self):
        ...
        intrinsic = tk.LabelFrame(left, text="Intrinsic")
        intrinsic.pack(side="left", fill="both", expand=True, padx=2)
        extrinsic = tk.LabelFrame(left, text="Extrinsic")
